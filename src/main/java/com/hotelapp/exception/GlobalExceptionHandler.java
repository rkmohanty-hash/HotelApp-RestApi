...
//when you talk about exception it is not only custom exception you may will  get different type of exception example 
//1.may be pathavariable is wrong.like that many exception example are there so spring provide ResponseEntityExceptionHandler abstract class
@ControllerAdvice
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleHttpMediaTypeNotSupported(HttpMediaTypeNotSupportedException ex,
		...
		String message=ex.getMessage();
		List<String> details=new ArrayList<>();
		details.add("Method type -- not supported");
		ApiErrors errors=new ApiErrors(message,details,status,LocalDateTime.now());
		return ResponseEntity.status(status).body(errors);
	}
